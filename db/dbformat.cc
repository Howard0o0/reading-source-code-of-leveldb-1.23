...
/* 将 Sequence Number 和 Value Type 打包得到一个 8 字节的整型，其中低 8 位为
 * Value Type。 因此 Sequence Number 虽然是一个 uint64_t
 * 的无符号整型，但是其最大值只能取到 1 << 56 - 1，因为在 InternalKey
 * 的实现中，我们需要将 Sequence Number 和 Value Type 打包。尽管如此，Sequence
 * Number 的 最大值也是一个天文数字。*/
