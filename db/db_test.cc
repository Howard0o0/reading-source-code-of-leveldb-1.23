...
TEST_F(DBTest, ApproximateSizes_MixOfSmallAndLarge) {
    do {
        ...
        if (options.reuse_logs) {
            // Need to force a memtable compaction since recovery does not do
            // so.
    ...
    while (!t->state->stop.load(std::memory_order_acquire)) {
        ...
        if (rnd.OneIn(2)) {
            ...
            if (s.IsNotFound()) {
                ...
                // Check that the writer thread counter is >= the counter in the
                // value
                ASSERT_LEVELDB_OK(s);
                int k, w, c;
...
static bool CompareIterators(int step, DB* model, DB* db,
    ...
    if (ok) {
        ...
        for (auto kiter = seek_keys.begin(); ok && kiter != seek_keys.end();
            ...
            if (miter->value().compare(dbiter->value()) != 0) {
                std::fprintf(stderr,
                             "step %d: Seek value mismatch for key '%s': '%s' "
                             "vs. '%s'\n",
                             step, EscapeString(miter->key()).c_str(),
                             EscapeString(miter->value()).c_str(),
                             EscapeString(miter->value()).c_str());
