...
void FilterBlockBuilder::StartBlock(uint64_t block_offset) {
     ...
     * 2048， 即 2KB，filter_index 就表示需要创建多少个 Bloom Filter */
    uint64_t filter_index = (block_offset / kFilterBase);

    assert(filter_index >= filter_offsets_.size());

    /* filter_offsets_ 用于保存每个 Bloom Filter 的起始偏移量，因为每一个 Bloom
     * Filter 的 长度可能是不同的。*/
...
/* `GenerateFilter()` 方法用于构建一个具体的 Filter Block K，其内部将会调用
 * `policy_->CreateFilter` 创建一个布隆过滤器，并将结果位数组（string 实现）返回
 */
