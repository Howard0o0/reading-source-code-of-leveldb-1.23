...
class Stats {
   ...
   public:
    ...
    void Report(const Slice& name) {
        ...
        std::string extra;
        if (bytes_ > 0) {
            // Rate is computed on actual elapsed time, not the sum of
...
class Benchmark {
   private:
    ...
    void PrintWarnings() {
        ...
        std::fprintf(stdout,
                     "WARNING: Optimization is disabled: benchmarks "
                     "unnecessarily slow\n");
#endif
#ifndef NDEBUG
   ...
   private:
    ...
    void RunBenchmark(int n, Slice name, void (Benchmark::*method)(ThreadState*)) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            arg[i].shared = &shared;
            ++total_thread_count_;
            // Seed the thread's random state deterministically based upon
