...
inline void EncodeFixed32(char* dst, uint32_t value) {
     ...
     *
     * static_cast 主要用于"相关类型"之间的强制类型转换，比如 double 转
     * int，double 转 float。
     * 相关类型是指转换类型和待转换类型之间多少还有点儿关系，比如 double 和
     * int，它们都属于数字。 又比如将 void * 指针转换成其它类型的指针，比如 int
     * *，这也没有问题。 但是如果把一个 long long
     * 类型转换成一个指针类型，这就不属于"相关类型" 之间的范畴了。
     *
     * 另外一点就是基类指针转换成派生类指针时，虽然它们确实有关系，但是应该使用
     * dynamic_cast 完成。
     *
     * reinterpret_cast 则用于"完全不相关类型"之间的强制类型转换。reinterpret
     * 可以翻译为重新释义，也就是
     ...
     * 并且可以看到，buffer[0] 写入的是 value 低 8 位，buffer[1]
     * 写入的是第二个低 8 位。
     * 因此，数据存放的方式是按照先低位后高位的顺序存放的，也就是说，leveldb
     * 采用的是小端存储（Little-Endian）*/
